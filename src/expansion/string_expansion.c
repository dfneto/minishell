...
t_token	*expand1(t_token *token) //$a$a$a
{
	...
	while (token && i < j) // agora vou expandir cada split_token
	{
		...
		token = last_token->next;
			// ou token = next token do ultimo token expandido
		i++;
	}
	return (token);
}
	...
	while (i < ft_strlen(token_str))
	{
		if (token_str[i] != ' ')
		{
			...
			while (token_str[i] && token_str[i] != ' ')
				...
				i++;
			if (!token->str) // so entra a primeira vez para cada split_token
				token->str = ft_strjoin(pre_dolar, ft_substr(token_str, start, i
							- start));
			else
			{
				new_token = create_token(token_str, start, i - 1, STRING);
...
t_token	*expand2(t_token *token)
{
	...
	// printf("pre-dolar: %s\n", pre_dolar);
	token->str = ft_substr(token->str, dolar_position, ft_strlen(token->str)
			- ft_strlen(pre_dolar));
	// inicialização
	i = 0;
	new_token = NULL;
...
/* Casos que devem passar
	...
	hola$a - ok
	hola$USER$USER
		-> deve ter o mesmo comportamento de quandao esta entre "": get_expanded_token(token)
	$USER$USER - ok
